...
function Protect-Bruteforce {
    ...
    begin {
        ...
        #Region Locale
        $PathToModule = Split-Path (Get-Module -ListAvailable Protect-Bruteforce).path
    ...
    process {
        ...
        $UserIPs | ForEach-Object {
            $Report = [Report]::new()
            $Report.IpAddress = [System.Net.IPAddress]::Parse($_.Name)
        ...
        if ($RDP) {
            ...
            if ($null -eq $RuleDisplayName) {
                ...
                if ($PSCmdlet.ShouldProcess($Verb, $BlockList.IpAddress, 'Protect-Bruteforce')) {
                    Write-Verbose $Verb
